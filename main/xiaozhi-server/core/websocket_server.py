...
class WebSocketServer:
    def __init__(self, config: Config):
        ...
        components = Components.initialize(config)
        self._vad = components.vad
        self._asr = components.stt
    ...
    async def _handle_connection(self, websocket):
        ...
        if headers.get("device-id", None) is None:
            ...
            if not request_path:
                logging.bind(tag=TAG).error("无法获取请求路径")
        ...
        try:
            ...
            logging.bind(tag=TAG).error(f"处理连接时出错: {e}")
        finally:
            # 确保从活动连接集合中移除
        ...
        try:
            async with self.config_lock:
                ...
                if new_config is None:
                    logging.bind(tag=TAG).error("获取新配置失败")
                    return False
                logging.bind(tag=TAG).info(f"获取新配置成功")
                # 检查 VAD 和 ASR 类型是否需要更新
                update_vad = check_vad_update(self.config, new_config)
                update_asr = check_asr_update(self.config, new_config)
                logging.bind(tag=TAG).info(
                ...
                logging.bind(tag=TAG).info(f"更新配置任务执行完毕")
                return True
        except Exception as e:
            logging.bind(tag=TAG).error(f"更新服务器配置失败: {str(e)}")
            return False
