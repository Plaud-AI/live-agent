...
-- ==================== Table: user ====================
-- User account table (supports email, Google, Apple authentication)
...
COMMENT ON TABLE "user" IS 'User account information (supports multiple auth providers)';
...
COMMENT ON COLUMN "user".password IS 'Hashed password (nullable for OAuth users)';
...
COMMENT ON COLUMN "user".display_name IS 'User display name';
COMMENT ON COLUMN "user".avatar_url IS 'User avatar URL';

-- ==================== Table: user_oauth ====================
-- OAuth provider connections for users (supports Google, Apple, Email)
...
-- Refresh tokens for extended user sessions
...
CREATE TABLE IF NOT EXISTS chat_messages (
    ...
    message_time TIMESTAMPTZ NOT NULL DEFAULT (CURRENT_TIMESTAMP AT TIME ZONE 'UTC'),
    created_at TIMESTAMPTZ NOT NULL DEFAULT (CURRENT_TIMESTAMP AT TIME ZONE 'UTC'),
...
CREATE INDEX IF NOT EXISTS idx_chat_messages_agent_message_time ON chat_messages(agent_id, message_time);
...
COMMENT ON COLUMN chat_messages.created_at IS 'When message was stored in database (UTC)';
...
-- Create triggers for updated_at on user_oauth table
